...
bool ISODUAL_TABLE::IsPositive(const TABLE_INDEX it, const int iv) const
{
  const TABLE_INDEX mask = (TABLE_INDEX(1) << iv);
...
(const bool flag_separate_neg, const bool flag_separate_opposite)
{
  ...
  for (TABLE_INDEX ientry = 0; ientry < NumTableEntries(); ientry++) {
    ...
    // *** ERROR. num_vertices should be 0 IF ALL VERTICES POS/NEG ***
...
// Constructor
ISODUAL_CUBE_TABLE_AMBIG::ISODUAL_CUBE_TABLE_AMBIG()
{
  Init();
}

// Constructor
ISODUAL_CUBE_TABLE_AMBIG::ISODUAL_CUBE_TABLE_AMBIG(const int dimension)
{
  Init();
  Create(dimension);
}
...
{
  Init();
  Create(dimension, separate_neg, flag_separate_opposite);
}

void ISODUAL_CUBE_TABLE_AMBIG::Init()
{
  is_ambiguous = NULL;
...
/// Free all memory.
void ISODUAL_CUBE_TABLE_AMBIG::FreeAll()
{
  delete [] is_ambiguous;
  is_ambiguous = NULL;
...
void ISODUAL_CUBE_TABLE_AMBIG::Alloc()
{
...
void ISODUAL_CUBE_TABLE_AMBIG::ComputeAmbiguityInformation()
{
  const int num_cube_facets = IJK::compute_num_cube_facets(dimension);
  FIND_COMPONENT find_component(dimension);

  for (TABLE_INDEX ientry = 0; ientry < num_table_entries; ientry++) {
...
void ISODUAL_CUBE_TABLE_AMBIG::ComputeNumActiveFacets()
{
  ISODUAL_CUBE_FACE_INFO<int,int,int> cube_face_info(dimension);

  for (TABLE_INDEX ientry = 0; ientry < num_table_entries; ientry++) {
    num_active_facets[ientry] = 
...
// Create table.
void ISODUAL_CUBE_TABLE_AMBIG::Create(const int dimension)
{
  // Create table.
  ISODUAL_CUBE_TABLE::Create(dimension);

  // Allocate memory for ambiguity information.
  Alloc();
...
// Create table.
void ISODUAL_CUBE_TABLE_AMBIG::Create
(const int dimension, const bool flag_separate_opposite)
{
  // Create table.
  ISODUAL_CUBE_TABLE::Create(dimension, flag_separate_opposite);
...
{
  // Create table.
  ISODUAL_CUBE_TABLE::Create
    (dimension, flag_separate_neg, flag_separate_opposite);
...
(const TABLE_INDEX ientry, 
 const ISODUAL_CUBE_TABLE_AMBIG::FACET_INDEX & kf, 
 FIND_COMPONENT & find_component)
{
  int num_pos_components = 
 ...
 FIND_COMPONENT & find_component)
{
  ...
  ISODUAL_CUBE_TABLE_AMBIG::FACET_SET mask = 
    ISODUAL_CUBE_TABLE_AMBIG::FACET_SET(1);

  for (ISODUAL_CUBE_TABLE_AMBIG::FACET_INDEX kf = 0; kf < num_facets; kf++) {
    if (is_cube_facet_ambiguous(ientry, kf, find_component)) {
      num_ambiguous_facets++;
      facet_set = (facet_set | mask);
    }
    mask = (mask << ISODUAL_CUBE_TABLE_AMBIG::FACET_INDEX(1));
  }
}
